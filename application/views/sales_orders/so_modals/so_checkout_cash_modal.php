...
<script>
    $(document).ready(function(){
        $('#add_payment_so').click(function(){ 
            ...
            $('#total_amount').val($('#invoice_total_fin').val()); 
        });
...
//            fl_alert('info',rowCount);
            if(this.id=='confirm_checkout_cash'){
                var pay_res = make_so_payments(amount,'cash');
                if(pay_res !='0'){ 
...
//                    
                    var new_trns_row_str = '<tr>'+
                                             '<td colspan="9"></td>'+
                                              '<td align="left"><span class=""> Cash Payment</span> </td>'+
...
//                            console.log(result1);
        ...
        function make_so_payments(amount,pay_method){
            $.ajax({
                           url: "<?php echo site_url('Sales_orders/fl_ajax');?>",
                           type: 'post',
                           data : {function_name:'add_so_payments', amount:amount,pay_method:pay_method, order_id:$('input[name=id]').val(),pay_method_id:$('#payment_method_id').val()},
