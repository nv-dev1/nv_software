...
class Payments extends CI_Controller {
        ...
        public function index($person_type=20){
            $this->add($person_type);
	} 
        
	function add($person_type=10){  //10: cust, 20:sup, 30: consignee
            $data['action']		= 'Add';
            $data['main_content']='payments/manage_payments'; 
                ...
                foreach ($inputs['allocation'] as $trans_ref){ 
                    if($trans_ref['amount'] != 0){
                        $data['trans_ref'][] = array(
                                                   ...
                                                   'trans_reference'=>$trans_ref['inv_no'],
...
//            echo '<pre>';            print_r($data); die;
		...
		if($add_stat[0]){ 
                    ...
                    redirect(base_url($this->router->fetch_class().'/add/'.$inputs['person_type'])); 
                }else{
...
//            echo '<pre>';            print_r($data); die;
        ...
        function get_purchasing_invoice_info($inv_id){
            if($inv_id!=''){
...
//            echo '<pre>';            print_r($this->input->post()); die;
            ...
            switch ($person_type){
                ...
                case 20: 
                    ...
                    $dues = $this->Purchasing_invoices_model->search_result('','payment_settled=0 AND i.supplier_id='.$person_id); 
                    foreach ($dues as $due_inv){
                        $data_arr = $this->get_purchasing_invoice_info($due_inv['id']);
                        $data_arr['invoice_dets']['invoice_no']= $data_arr['invoice_dets']['supplier_invoice_no']; 
                        $data_arr['trans_type'] = 'Purchasing Invoice';
                        $due_info[] =  $data_arr;
                    }
                    break;
            }
