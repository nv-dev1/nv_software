...
class Sales_commissions extends CI_Controller {
...
//            $data['search_list'] = $this->Sales_invoices_model->search_result();
            $data['main_content']='reports_all/sales/sales_commissions/search_sales_commissions_report'; 
            $data['consignee_list'] = get_dropdown_data(CONSIGNEES,'consignee_name','id','No Consignee');
...
//            echo '<pre>';            print_r($this->input->post()); die;
        ...
        public function search(){ //view the report
            ...
            $this->load->view('reports_all/sales/sales_commissions/search_sales_commissions_report_result',$data);
	} 
        
        public function print_report(){ 
//            $this->input->post() = 'aa';
            $item_commish_data = $this->load_data(); 
...
//            echo '<pre>';            print_r($input_get); die; 
            ...
            $pdf->fl_header_title_RTOP='Consignee/ Broker Commissions';//invice bg
        ...
        //        $this->writeHTMLCell(90,20,60,23,$header_info); 
            ...
            $html .= '<table id="example1" class="table-line" border="0">
                                                    <thead> 
                                                        <tr style="">  
                                <th width="3%" align="center"><b>#</b></th> 
                                <th width="10%" align="center"><b>Consignee</b></th>
                                <th width="7%" align="center"><b>Code</b></th> 
                                <th width="15%" align="center"><b>Desc</b></th> 
                                <th width="10%" align="center"><b>Date</b></th> 
                                <th width="12%" align="center"><b>Invoice No</b></th> 
                                <th width="12%" align="center"><b>CR No</b></th> 
                                <th width="12%" align="center"><b>Units</b></th> 
                                <th width="13%" align="right"><b>Commissions Type</b></th>  
                                <th width="12%" align="right"><b>Commission</b></th>
                            </tr> 
                        </thead>
                        <tbody>';  
            $i=1; 
            
            foreach ($item_commish_data as $item){
//            echo '<pre>';            print_r($item); die;
              
                           $html .= '<tr>
                                        <td width="3%" align="center">'.$i.'</td>
                                        <td width="10%" align="center" >'.$item['consignee_name'].'</td>
                                        <td width="7%" align="center">'.$item['item_code'].'</td>
                                        <td width="15%" align="center">'.$item['item_name'].'</td>
                                        <td width="10%" align="center">'. date(SYS_DATE_FORMAT,$item['invoice_date']).'</td>
                                        <td width="12%" align="center">'.$item['invoice_no'].'</td>
                                        <td width="12%" align="center">'.$item['cr_no'].'</td>
                                        <td width="12%" align="center">'.$item['commision_unit'].' '.$item['uom_name'].' '.(($item['commision_unit_2']>0)?' | '.$item['commision_unit_2'].' '.$item['uom_name_2']:'').'</td> 
                                        <td width="13%" align="right">'.(($item['consignment_type_id']==1 || $item['consignment_type_id']==2)?'Percentage ('.$item['consignment_rate'].'%)':'Fixed Amount').'</td>
                                        <td width="12%" align="right">'. number_format($item['consignment_amount'],2).'</td>
...
//            echo '<pre>';            print_r($html); die;
            $pdf->writeHTMLCell(190,'',5,(($first_page_header_only==1)?50:''),$html);
...
//            echo '<pre>';            print_r($input); die;  
            ...
            $commish_data = $this->Reports_all_model->get_consignee_commisions($input);
